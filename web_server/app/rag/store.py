...
def index_transcript_segments(
    ...
    """Index transcript segments into RAG."""
    if not timestamps:
        return
...
def index_meeting(
    ...
    """Index one meeting's main topic and subtopics into RAG."""
...
def search_transcript_segments(
    ...
    """Search transcript segments. Returns list of { text, job_id, start, end, original_filename, distance }."""
...
def search_meetings(
    ...
    """Search meetings by topic. Returns list of { job_id, original_filename, main_topic, document, distance }."""
    limit = limit or RAG_SEARCH_LIMIT
    coll = _get_meetings_collection()
    if coll.count() == 0:
        return []
